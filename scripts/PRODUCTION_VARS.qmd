...
```{r potential}
# Load data
load(here('data', 'land_pty_dynamics.RData')) # land productivity dynamics
load(here('data', 'biodiversity_intactness.RData')) # bidiversity intactness index
...
potential <- croplands %>% 
  ...
  left_join(land_pty_dynamics %>% st_drop_geometry() %>% 
            select(grappe, land_pty_dynamics),
            by = 'grappe') %>% 
  left_join(biodiversity_intactness %>% st_drop_geometry() %>% 
            select(grappe, biodiversity_intactness),
            by = 'grappe') %>% 
...
availability <- auto_conso_by_grappe %>% 
  ...
  left_join(crop_production %>% 
              select(grappe, crop_production_pmil, crop_production_vege) %>%
              st_drop_geometry(),
            by = "grappe") %>% 
  replace_na(list(crop_production_pmil = 0, 
                  crop_production_vege = 0)) %>% 
  left_join(food_prod_kcal %>% 
              select(grappe, food_prod_kcal) %>% 
              st_drop_geometry(),
            by = "grappe") %>% 
  replace_na(list(food_prod_kcal = 0)) %>% 
  left_join(feed_prod_kcal %>% 
              select(grappe, feed_prod_kcal) %>% 
              st_drop_geometry(),
            by = "grappe") %>% 
  replace_na(list(feed_prod_kcal = 0))
