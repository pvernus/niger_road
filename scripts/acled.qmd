...
ner_acled_grappes <- west_africa_acled_sf %>%
  ...
  mutate(grappe = st_nearest_feature(., grappe_sf, check_crs = TRUE))

freq_vio_events_by_grappe <- ner_acled_grappes %>% 
  st_drop_geometry() %>% 
...
save(freq_vio_events_by_grappe, file = here('data', 'freq_vio_events.RData'))
