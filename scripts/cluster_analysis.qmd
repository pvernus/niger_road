...
indicator_balancing <- column_to_rownames(cluster_data, var = "grappe") %>% 
  select(biodiversity_intactness, water_depletion, croplands_2019_netgain,
        ...
        empl_food_agriculture, empl_food_processing, empl_food_marketing,
        prod_stock, # availability
        price_diff_mean, price_change_season_mean, pov, pastoral, landless,
        distance_main_road, distance_marketplace, car, freq_vio_events, # access
        varied_diet, cooking_stove, fridge, waste # utilization
  )
...
mean_total <- bind_rows(map(cluster_norm_balanced, mean)) |> 
  ...
  mutate(
    dimension = fct_relevel(as_factor(
      ...
      name %in% c('prod_stock', 'empl_food_agriculture', 'empl_food_processing', 'empl_food_marketing', 'livestock_tlu_avg', 'crop_production_pmil', 'crop_production_vege') ~ 'Disponibilité',
      name %in% c('price_diff_mean', 'price_change_season_mean', 'distance_marketplace', 'distance_main_road', 'pov', 'freq_vio_events', 'landless', 'pastoral', 'car') ~ 'Accès',
      name %in% c('varied_diet', 'fridge', 'waste', 'cooking_stove') ~ 'Utilisation'
  )),
...
lapply(mean_cluster_list, function(df)
  df %>% 
    mutate(name = case_when(
      ...
      name == 'croplands_2019_netgain' ~ 'Gain net de terres cultivées 2000-2029',
      name == 'land_pty_dynamics' ~ 'Dynamique de la productivité des sols',
      name == 'prod_stock' ~ 'Production agricole stockée',
      name == 'empl_food_agriculture' ~ 'Emploi agro-alimentaire - Production',
      name == 'empl_food_processing' ~ 'Emploi agro-alimentaire - Transformation',
      name == 'empl_food_marketing' ~ 'Emploi agro-alimentaire - Commercialisation',
      ...
      name == 'price_diff_mean' ~ 'Variation des prix locaux / moyenne nationale',
      name == 'price_change_season_mean' ~ 'Variation saisonnière des prix locaux',
      name == 'pov' ~ 'Pauvreté monétaire',
      name == 'landless' ~ 'Population sans-terre',
      name == 'pastoral' ~ 'Population pastorale',
      name == 'distance_main_road' ~ 'Distance - Route principale',
      name == 'distance_marketplace' ~  'Distance - Marché',
      name == 'freq_vio_events' ~ 'Conflits violents',
      name == 'car' ~ 'Voiture',
      name == 'varied_diet' ~ 'Régime alimentaire varié',
      name == 'fridge' ~ 'Frigidaire',
      name == 'waste' ~ 'Egouts', 
      name == 'cooking_stove' ~ 'Cuisinière',
    )) %>% 
  ggplot() +
    ...
    coord_flip() +
    scale_fill_manual(values = c('#C4EAFF', '#005DB9', '#7030A0', '#EFE5F7')) +
    labs(x = "Indicateurs", y = "Déviation du profil moyen") +
    theme_minimal()
  )
