...
+==============+=====================================================================================+===========================+=============+
| Potential    | Agroecological zones (binomial)                                                     | zae                       | EHCVM 2018  |
|              |                                                                                     |                           |             |
|              | Cropland extent                                                                     |                           |             |
+--------------+-------------------------------------------------------------------------------------+---------------------------+-------------+
...
cultivated_area <- hh_food_prod %>%
  ...
  ungroup() %>% 
  mutate(
    hh_total_superf = if_else(is.na(hh_total_superf), 0, scale(hh_total_superf)),
...
fviz_dend(data_tree, k = 4, # cut in four groups
     cex = .5, # label size
     horiz = TRUE,
...
mean_cluster_list <- lapply(mean_cluster_list, function(df) mutate(df, diff = value.total - value.cluster))

# deviation from average for all variables for each cluster
lapply(mean_cluster_list, function(df) df %>% 
