...
cluster_survey_data <- merged_data |> 
  ...
  summarize(
    ...
    hh_crop_diversity = mean(crop_diversity, na.rm = TRUE), # auto-production
...
cluster_b <- column_to_rownames(cluster_data, var = "grappe") %>% 
  select(-c(crop_production_cowp, crop_production_sorg, crop_production_total, hh_cultivated_area))
cluster_norm_b <- scale(cluster_bal) |> 
  as.data.frame()

corr_matrix_b <- cor(scale(cluster_bal))
score_matrix_b <- score_matrix(cluster_norm_bal)
```
...
eig <- get_eig(data_pca) %>% rownames_to_column() %>% select(pc = rowname, cumulative.variance.percent) %>% 
  ...
  filter(!pc %in% c('Dim.11', 'Dim.12', 'Dim.13', 'Dim.14', 'Dim.15')) %>% 
  mutate(pc = as_factor(pc))

ggplot(data=eig, aes(x=pc, y=value, fill = pc)) +
geom_bar(stat="identity", color="black", position=position_dodge()) +
  scale_fill_manual(values=c("lightblue", "lightblue", "lightblue",  "lightblue", "red",
                             "lightblue",  "lightblue", "lightblue",  "lightblue","lightblue")) +
  geom_hline(yintercept=66, linetype="dashed", color = "red") +
...
# Clusters = 6
png(file=here('outputs', 'hclust_6.png'),
width=700, height=500)
# graph visualization
require("igraph")
fviz_dend(data_tree, k = 6, 
     cex = .5, # label size
     horiz = TRUE,
     k_colors = "jco", # other option: c("#2E9FDF", "#00AFBB", "E7B800", "#FC4E07")
     rect = TRUE,
