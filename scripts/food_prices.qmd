...
food_prices_avg_year_deviation <- food_prices %>% 
  ...
  mutate(mean_month = mean(price), # mean price by month for each commodity
         dev_mean_month = 100 * (price - mean_month) / mean_month,
         median_month = median(price),
         dev_median_month = 100 * (price - median_month) / median_month
         ) %>% 
  ungroup()

price_avg_year_deviation <- food_prices_avg_year_deviation %>% 
  summarize(dev_mean_month = median(dev_mean_month, na.rm = TRUE), 
            dev_median_month = median(dev_median_month, na.rm = TRUE),
            .by = market)
```
...
seasonality_year <- food_prices %>% 
  ...
  mutate(price_change_season = round(100 * (wet - dry) / dry, 2)) # variation rate

seasonality <- seasonality_year %>% 
  summarize(price_change_season = median(price_change_season, na.rm = TRUE), .by = market)
...
# nngeo::st_nn(grappe_sf, price_market_sf, k = 2)
st_join(grappe_sf, price_market_sf, join = nngeo::st_nn, k = 1, progress = FALSE) # k = 2 for less sensitivity?
